                ...
                if(eol=="EOL-YRA-FORK03-12"):
                    ...
                    while (row_index <= total_row):
                        try:
                            ...
                            d1[22]=str(d1[22]).strip()
                            d1[23]=str(d1[23]).strip()
                            #d1[37]=str(d1[24]).strip()
                            eol_time=d1[1].strftime("%H:%M:%S")
                            sql_select_query='SELECT * FROM public.yra_fork3_12 WHERE eol_serial='+"'"+str(d1[0])+"' AND production_date="+"'"+str(cdate)+"' AND eol_check_time="+"'"+str(eol_time)+"';"
                            sql_insert_query = 'INSERT INTO public.yra_fork3_12(eol_serial, production_date, eol_check_time, "M01", "M02", "M03", "M04", "M05", "M06", "M07", "M08", "M09", "M10", "M11", "M12", "M13", "M14", "M15", "M16", "M20", result1, pinmark_no)  VALUES('+str(d1[0])+",'"+cdate+"','"+eol_time+"',"+str(d1[2])+","+str(d1[3])+","+str(d1[4])+","+str(d1[5])+","+str(d1[6])+","+str(d1[7])+","+str(d1[8])+","+str(d1[9])+","+str(d1[10])+","+str(d1[11])+","+str(d1[12])+","+str(d1[13])+","+str(d1[14])+","+str(d1[15])+","+str(d1[16])+","+str(d1[17])+","+str(d1[21])+",'"+str(d1[22])+"','"+str(d1[23])+"');"
