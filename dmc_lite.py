#DMC Lite v . 1 Author: RAKESH RANJAN JENA, IMS , BEST KOKI, EMail: rrjprince@rrjprince.com
...
try:
    dmc_process_log="DMC_Process_log/dmc_lite_process_log.txt"
...
def errLog(err_type,err_data):
    try:
        ...
        dmc_error_log_file_name="DMC_Error_log/Error_log_dmc_lite_protocol-"+str(log_date)+".log"
...
def org_docs(path):
    ...
    for n in bkp_files_path:
        ...
        if(n[0]==" "):
            ...
            errLog("\n NEW ERROR FOLDER NAME GONE DMC PROTOCOL :",str(n)+"\\"+str(eol_file_dir)+"\n")
            processLog("\n NEW FOLDER FOR DMC PROTOCOL :"+str(n)+"\\"+str(eol_file_dir)+"\n")
            ...
            if(len(bkp_files_array)==0):
                try:
                    ...
                    print(e)
                    errLog("EOL BACKUP EMPTY DIR REMOVAL ERROR: ",str(e))
                empty_dir_count+=1
            ...
            #UPDATE BACKUP DETAILS IN EOL BACKUP DETAILS TABLES
            try:
                conn = sqlite3.connect('EOLDMCDATA.db')
...
#CALLING ALL ABOVE PROCESS
start_time = time.time()
processLog("\n ------------------------DMC PROCESS STRTED AT: "+str(datetime.date)+str(start_time)+"---------------------------------------- \n")
try:
    ...
    conn.execute("INSERT INTO process_log(start_date,start_time,task_type) VALUES(?,?,?)",(str(db_start_date),str(db_start_time),"DMC-LITE"))
    conn.commit()
    conn.close()
...
try:
    ...
    conn.execute("UPDATE process_log SET end_date='"+str(db_end_date)+"',end_time='"+str(db_end_time)+"',time_taken="+str(int(end_time))+" WHERE start_date='"+str(db_start_date)+"' AND start_time='"+str(db_start_time)+"' AND task_type="+"'DMC-LITE'")
    conn.commit()
    conn.close()
